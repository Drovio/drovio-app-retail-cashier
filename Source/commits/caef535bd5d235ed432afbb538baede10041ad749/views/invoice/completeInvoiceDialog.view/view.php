...
if (engine::isPost())
{
	...
	$pbuttonContainer = DOM::create("div", "", "", "pbuttonContainer");
	$appContent->append($pbuttonContainer);
	
	// Create preview button with attributes
	$title = $appContent->getLiteral("main.invoice", "lbl_previewFile");
	$dlButton = DOM::create("div", $title, "btn_preview");
	HTML::addClass($dlButton, "pbutton positive");
	$attr = array();
	$attr['iid'] = $invoiceID;
	$actionFactory->setAction($dlButton, $viewName = "invoice/previewInvoicePDF", "", $attr);
	DOM::append($pbuttonContainer, $dlButton);
	...
	HTML::addClass($dlButton, "pbutton");
	$attr = array();
	$attr['iid'] = $invoiceID;
