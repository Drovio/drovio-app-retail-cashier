...
else
{
	...
	foreach ($allProducts as $productInfo)
	{
		...
		$fld = DOM::create("div", $productInfo['discount']." €", "", "fld pdiscount");
		DOM::append($pview, $fld);
		...
		$totalPrice = number_format($productInfo['product_price'] * (1 + $productInfo['tax_rate']) * $productInfo['amount'] - $productInfo['discount'], 2);
		$fld = DOM::create("div", $totalPrice." €", "", "fld ptotal_price");
		DOM::append($pview, $fld);
		...
		// Inputs
		$input = $form->getInput($type = "text", $name = "price", $value = $productInfo['product_price'], $class = "fld pprice", $autofocus = FALSE, $required = TRUE);
		$form->append($input);
		...
		$input = $form->getInput($type = "text", $name = "discount", $value = $productInfo['discount'], $class = "fld pdiscount", $autofocus = FALSE, $required = TRUE);
		$form->append($input);
		
		$totalPrice = number_format($productInfo['product_price'] * (1 + $productInfo['tax_rate']) * $productInfo['amount'] - $productInfo['discount'], 2);
		$label = $form->getLabel($text = $totalPrice, $for = "", $class = "fld ptotal_price");
		$form->append($label);
