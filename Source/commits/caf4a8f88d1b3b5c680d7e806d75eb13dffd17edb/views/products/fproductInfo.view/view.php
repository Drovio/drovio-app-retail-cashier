...
if (empty($prices))
{
	// No prices notification
	$title = $appContent->getLiteral("products.dialog", "lbl_noPrice");
	$ntf = DOM::create("h2", $title, "", "hd_ntf");
	$appContent->append($ntf);
}
else
{
	$priceResource = array();
	foreach ($prices as $type_id => $price)
		$priceResource[$price] = $priceTypes[$type_id]." (".$price." €)";
	...
	// Price
	$priceValue = array_values($prices)[0];
