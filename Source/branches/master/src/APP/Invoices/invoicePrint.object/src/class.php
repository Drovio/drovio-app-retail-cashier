...
application::import("Settings", "appSettings");
...
class invoicePrint
{
	...
	public function __construct($invoiceID = "")
	{
		...
		// Get print literals
		$this->literals = appLiteral::get("invoice.print");
	...
	private function createInvoicePDF($doubleCopy = FALSE)
	{
		...
		{
			// Print original
			$this->printInvoiceCopy($label = $this->literals['label.copy.prototype']);
			
			// Print copy
			$this->printInvoiceCopy($label = $this->literals['label.copy.copy']);
		}
		else
		{
			// Print original only
	...
	private function printInvoiceCopy($copyLabel = "")
	{
		...
		for ($i = 0; $i < $numberOfPages; $i++)
		{
			...
			// Print current page
			$pdf->SetXY(10, 270);
			$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['page']." ".($i + 1)."/".$numberOfPages);
		}
		...
		$anchorX = 125;
		$pdf->SetXY($anchorX, $anchorY);
		$pdf->Cell($w = 200, $h = 5, $this->literals['total_amount'].": ".number_format($totalAmount, 2));
	...
	private function printCompanyInfo()
	{
		...
		$printHeader = $this->settings->get("print_header");
		$descLines = explode("\n", $printHeader);
	...
	private function printInvoiceInfo($pdf)
	{
		...
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['invoiceinfo.invoice_type'], $border = FALSE, $ln = 25, $align = "C");
		$pdf->SetXY(10, $anchorY + 5);
		$pdf->Cell($w = 100, $h = 5, $txt = $invoiceInfo['type'], $border = FALSE, $ln = 25, $align = "C");
		
		$pdf->SetXY(100, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.invoice_number'], $border = FALSE, $ln = 25, $align = "C");
		$pdf->SetXY(100, $anchorY + 5);
		$pdf->Cell($w = 50, $h = 5, $txt = $invoiceInfo['invoice_id'], $border = FALSE, $ln = 25, $align = "C");
		
		$pdf->SetXY(150, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.invoice_date'], $border = FALSE, $ln = 25, $align = "C");
		...
		$pdf->SetXY(10, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.way_of_payment'], $border = FALSE, $ln = 25);
		$pdf->SetXY(60, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $invoiceInfo['way_of_payment'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(10, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.purpose_of_trafficking'], $border = FALSE, $ln = 25);
		$pdf->SetXY(60, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $invoiceInfo['purpose_of_trafficking'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(10, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.way_of_shipping'], $border = FALSE, $ln = 25);
		$pdf->SetXY(60, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $invoiceInfo['way_of_shipping'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(10, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.shipping_location'], $border = FALSE, $ln = 25);
		$pdf->SetXY(60, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $invoiceInfo['shipping_location'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(10, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['invoiceinfo.delivery_location'], $border = FALSE, $ln = 25);
		$pdf->SetXY(60, $anchorY);
	...
	private function printCustomerInfo()
	{
		...
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.id'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $customerInfo['person_id'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.name'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		...
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.occupation'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		...
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.address'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $address['address'].", ".$address['postal_code'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.city'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		...
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.phone'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $phone['phone'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.tax_code'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
		$pdf->Cell($w = 57.5, $h = 5, $txt = $customerInfo['tax_id'], $border = FALSE, $ln = 25);
		
		$anchorY += $lineHeight;
		$pdf->SetXY(110, $anchorY);
		$pdf->Cell($w = 50, $h = 5, $txt = $this->literals['customerinfo.irs'], $border = FALSE, $ln = 25);
		$pdf->SetXY(135, $anchorY);
	...
	private function printProductInfo()
	{
		...
		$anchorX = 5;
		$pdf->SetXY($anchorX, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.id']);
		$pdf->SetXY($anchorX += 20, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.description']);
		$pdf->SetXY($anchorX += 60, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.munit']);
		$pdf->SetXY($anchorX += 20, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.amount']);
		$pdf->SetXY($anchorX += 20, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.price_per_unit']);
		$pdf->SetXY($anchorX += 20, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.discount_perc']);
		$pdf->SetXY($anchorX += 20, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.price']);
		$pdf->SetXY($anchorX += 20, $anchorY);
		$pdf->Cell($w = 100, $h = 5, $txt = $this->literals['productinfo.tax_perc']);
	}
	...
	private function printFooterInfo()
	{
		...
		$pdf->SetXY($anchorX, $anchorY);
		$pdf->Cell($w = 40, $h = 5, $txt = $this->literals['footer.total_value'], $border = FALSE, $ln = 25, $align = "C");
		$pdf->SetXY($anchorX += 40, $anchorY);
		$pdf->Cell($w = 40, $h = 5, $txt = $this->literals['footer.total_discount'], $border = FALSE, $ln = 25, $align = "C");
		$pdf->SetXY($anchorX += 40, $anchorY);
		$pdf->Cell($w = 40, $h = 5, $txt = $this->literals['footer.total_vat'], $border = FALSE, $ln = 25, $align = "C");
		...
		$pdf->SetXY($anchorX, $anchorY);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['footer.vat_perc']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['footer.vat_price']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['footer.vat_value']);
		...
		$pdf->SetXY($anchorX, $anchorY);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['balance.old']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['balance.payment']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['balance.new']);
		...
		$pdf->SetXY($anchorX, $anchorY);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.total_value']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.discount_perc']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.discount']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.clean_price']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.total_generic']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.vat']);
		$pdf->SetXY($anchorX, $anchorY += 5);
		$pdf->Rect($anchorX, $anchorY, 77.5, 0);
		$pdf->SetFontSize(11);
		$pdf->Cell($w = 10, $h = 5, $txt = $this->literals['totals.payment']);
		...
		$pdf->Cell($w = 40, $h = 5, $txt = $this->literals['signatures.issuer'], $border = FALSE, $ln = 5, $align = "C");
		$pdf->SetXY($anchorX += 40, $anchorY);
		$pdf->Cell($w = 40, $h = 5, $txt = $this->literals['signatures.recipient'], $border = FALSE, $ln = 5, $align = "C");
		
		// Add invoice 'disclaimer' information
		$anchorY = 245;
		$anchorX = 5;
		$pdf->SetFontSize(8);
		$printFooter = $this->settings->get("print_footer");
		$lines = explode("\n", $printFooter);
