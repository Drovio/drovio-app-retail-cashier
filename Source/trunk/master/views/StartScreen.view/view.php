...
$printHeader = $settings->get("print_header");
$printFooter = $settings->get("print_footer");
if (empty($companyName) || empty($companyName) || empty($companyName))
{
	// Set settings popup action
	$settingsButton = HTML::select(".cashierStartScreen .notification .sbutton.settings")->item(0);
	$actionFactory->setAction($settingsButton, "settings/appSettingsPopup");
}
else
{
	// Remove notification
	$notification = HTML::select(".cashierStartScreen .notification")->item(0);
	HTML::replace($notification, NULL);
}
